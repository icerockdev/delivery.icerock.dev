import i18n from 'roddeh-i18n';

export default i18n.create({
	values: {
		// base.hbs
		"Платформа для доставки товаров - мобильное приложение для доставки товаров, управление курьерами, мобильное приложение витрины товаров":
			"Online sales platform - mobile application for delivering orders, managing couriers and presenting goods",

		// title.hbs
		"Продавайте онлайн и сохраните клиентов": "Sell online and retain your customers",
		"c помощью мобильного приложения по заказу и доставке":
			"by means of the mobile app for goods order and delivery",
		"Разработка от 3-х дней. Коробочное решение в 3 раза дешевле индивидуального":
			"Three days development term (or little longer). “Box” (ready-maid or retail) solution costs three times less than the custom-designed one.",
		"Получить консультацию": "Get advice",

		// reasons.hbs
		"Как моему бизнесу поможет приложение?":
			"How does my business benefit from the app?",
		"Для ресторанов": "Restaurants",
		"Для аптек": "Pharmacies",
		"Для магазинов": "Shops",
		"Безопасность": "Safety",
		"Создаст безопасную альтернативу оффлайн-покупкам во время карантина":
			"Creates a secure alternative to offline shopping during period of quarantine",
		"Доставка": "Delivery",
		"Поможет наладить доставку &mdash; есть интеграция с Доставистой или другими местными сервисами":
			"Helps to arrange delivery – can be integrated with Dostavista or other local courier services",
		"Поиск клиентов": "Customers attraction",
		"Сохранит клиентов и поможет найти новых": "Retains customers and helps to find new ones",
		"Лояльность": "Loyalty",
		"Повысит лояльность клиентов &mdash; им удобно делать заказ, они видят, что в приложении есть контент, который интересно смотреть и читать":
			"Increases customer loyalty - they feel comfortable while ordering and enjoy watching and reading relevant content.",
		"Увеличение среднего чека": "Average check growth",
		"Увеличит средний чек и кол-во покупок за счёт индивидуальных подборок товаров и информирования об акциях, скидках, предложениях":
			"Increases purchase amount and number by means of personalized goods selection and informing on sales, discounts and special offers.",

		// graphs.hbs
		"Почему важно сделать приложение именно сейчас?":
			"Why is it important to get the app now?",
		"После снятия карантина поведение покупателей изменится. Люди поймут, что доставка &mdash; удобный сервис. Они привыкнут делать заказы онлайн. К тому времени многие компании уже сделают мобильные приложения и организуют доставку. Без этих сервисов сложно будет приспособиться к новым условиям и конкурировать за клиентов. А создавать и раскручивать их с нуля будет поздно.":
			"Buyers’ behavior will change after the quarantine ceases. Understanding advantages of delivery services is what to expect. Customers used to ordering online. By then, many companies will have mobile apps already made and delivery schemes arranged. Without these, you will find hard to adapt to new circumstances and compete for customers. Falling behind time is bad.",
		"./assets/fonts/graphs.svg": "./assets/images/graphs_en.svg",
		"./assets/fonts/graphs_mobile.svg": "./assets/images/graphs_mobile_en.svg",

		// interaction.hbs
		"Приложение позволяет постоянно взаимодействовать с клиентами":
			"Our application provides ongoing interaction between you and your clients.",
		"Уведомления": "Push notifications",
		"Они быстро получат уведомление о новинках. Ассортимент в приложении автоматически обновляется из вашей CRM":
			"Buyers receive prompt info on new products. The goods’ assortment in the app gets automatic updates by your CRM.",
		"Интересный контент": "Entertaining content",
		"В приложение можно добавить вовлекающий контент: видео-рецепты, обзоры, отзывы, видео с кухни":
			"You can add engaging content to your application such as video recipes, reviews, comments, chefs’ on-line lessons.",
		"Клиентская база": "Customer base",
		"Можно загрузить базу адресов. Вы будете информировать покупателей о скидках, акциях, индивидуальных предложениях":
			"You can import your own address database. Thus, keeping customers informed on sales, discounts, promotions and individual offers.",
		"Бонусная программа": "Bonus program",
		"Приложение позволит запустить программы лояльности: ввести баллы, промокоды":
			"The application lets you manage customer loyalty by awarding points or promo-codes.",

		// app.hbs
		"Мы сделаем простое и понятное приложение":
			"We make the application simple but handy and useful",
		"Им удобно пользоваться клиентам и вашим сотрудникам. В приложении есть три интерфейса:":
			"Both your customers and your employees can use it with ease. There are three interfaces:",
		"Для клиента": "Customer",
		"Для сборщика": "Packager",
		"Для курьера": "Courier",

		// how.hbs
		"Как это работает?": "How does it work?",
		"У пользователя в системе может быть 3 роли. Каждая роль может быть монетизирована, являться частью единой инфраструктуры или быть отдельным независимым подразделением или партнером":
			"User can play any of the three roles. Each role can be monetized; it can be a part of joint infrastructure, a separate independent unit or a partner",
		"Поиск товара": "Goods selection",
		"Оформление заказа и оплата": "Order processing and payment",
		"Сборка": "Packaging",
		"./assets/fonts/how_customer.svg": "./assets/images/how_customer_en.svg",
		"./assets/fonts/how_platform.svg": "./assets/images/how_platform_en.svg",
		"./assets/fonts/how_shop.svg": "./assets/images/how_shop_en.svg",
		"./assets/fonts/how_courier.svg": "./assets/images/how_courier_en.svg",

		// where.hbs
		"Где работает наше приложение?":
			"What OS does our app work on?",
		"Мы создаем приложения для IOS и Android":
			"We develop apps for iOS and Android",
		"Поддерживаем мультиплатформенную разработку на языке Kotlin.":
			"We use Kotlin multiplatform development.",
		"Он позволяет написать приложение на одном языке сразу под 8 платформ":
			"It allows creating one application for eight platforms at the same time.",
		"Заказать консультацию": "Get Advice",

		// risks.hbs
		"Мы предусмотрели все риски": "All the risks are considered",
		"У нас большой опыт разработки и готовые платформы. Мы в короткие сроки создаем приложение и настраиваем его под конкретный бизнес":
			"We have extensive development experience and ready-made platforms. We create apps rapidly and attune them for a specific business needs.",
		"Приложение быстро выпускаем на рынок &mdash; всего за 3 рабочих дня":
			"We bring the app into the market quickly, in three business days.",
		"Наши приложения работают в рамках законодательства. Мы учитываем все юридические нюансы, поэтому можно не беспокоиться о проблемах с налоговой службой и другими инстанциями":
			"Our apps fit the local legislation. We thoroughly take account of all legal aspects, so you can avoid issues with revenue service and other state authorities.",
		"Узнать подробности": "Find Out More",

		// aboutus.hbs
		"Кто мы?": "Who are we?\n",
		"Мы &mdash; небольшая команда специалистов из самого сердца Сибири, её технического центра &mdash; Новосибирска. Мы специализируемся на разработке мобильных приложений для Android и IOS. Наши знания и опыт позволяют нам эффективно решать задачи и создавать качественные, удобные	и эффективные приложения.":
			"We are 40 developers from the heart of Siberia, its engineering center — city of Novosibirsk. We focused on mobile apps for Android and iOS. Our knowledge and experience are strong enough to meet challenges and produce top-quality, convenient and efficient mobile applications.\n",
		"Александр Погребняк": "Aleksandr Pogrebynak",
		"Руководитель": "Director",
		"Валерий Гетманов": "Valeriy Ghetmanov",
		"Менеджер по работе с клиентами": "Customer Service Manager",
		"Алексей Михайлов": "Aleksey Mikhailov",
		"Технический директор": "Technical Director",
		"Евгений Гребенщиков": "Yevgeniy Grebenshchikov",
		"Арт-директор": "Art Director",
		"Андрей Чернов": "Andrey Chernov",
		"Руководитель iOS-отдела": "Head of iOS Team",
		"Алексей Шведов": "Aleksey Shvedov",
		"Руководитель Web-отдела": "Head of Web Team",

		// startup.hbs
		"У нас понятный и прозрачный цикл запуска": "We provide clear and transparent launch cycle",
		"Разработка приложения занимает от 3-х дней": "Software development takes three days (or little longer)",
		"Устанавливаем приложение": "Installation",
		"Запускаем": "Launch",
		"Предлагаем комплексное решение по маркетингу": "All-in-one marketing solution is available",
		"Cоздаем рассылку (серия писем по базе клиентов)": "Initial mailout throughout the customer list",
		"Делаем лендинг": "Landing pages",
		"Разработываем рекламные кампаний в Yandex и Google": "Advertising campaign in Yandex and Google",

		// showcase.hbs
		"Скоро запуск": "Launches soon",
		"Мы уже разработали приложения для": "We have developed the application for",
		"Сервис заказа домашней еды": "Home-made food delivery",
		"Приложение позволяет заказывать блюда в апартаменты":
			"The application allows you to order meals to your apartment",
		"Сервис моментальной доставки продуктов":
			"Instant product delivery service",

		// solution.hbs
		"Мы предлагаем &laquo;коробочное&raquo; решение в собственность или по подписке":
			"We are offering either software solutions for your personal use or by subscription",
		"Мобильное приложение с фирменным стилем": "Mobile app in your corporate style",
		"Данные и сервер находятся у разработчика": "Developer hosts all data and server",
		"Разработка от 3-х дней": "Three days development term (or little longer)",
		"Подключение 200 т.руб, ежемесячно 55 т.руб": "Initial cost RUB 200,000, monthly fee RUB 55,000",
		"&laquo;Коробочное решение&raquo;": "«Box» solution",
		"Создаем фирменное мобильное приложение": "Mobile app in your corporate style",
		"Сервер и все данные принадлежат вам": "You host all data and server",
		"Можете подключить любой сервис или интеграции": "You can integrate any external service",
		"Разработка от 2-х недель, стоимость: 1&minus;1,5 млн. руб":
			"Two weeks development term (or little longer), cost RUB 1.0 to 1.5 million rubles.",

		"Индивидуальная разработка": "Custom-designed application",
		"Разрабатываем с &laquo;нуля&raquo; именно под ваш бренд": "Developed «from the ground up» to meet your specific requirements",
		"Вы владеете сервером и данными": "You host all data and server",
		"Разработка от 4-х месяцев, стоимость от 3 млн.руб":
			"Four months development term (or little longer), cost RUB 3 million rubles or more",

		// modal.hbs
		"Пожалуйста, заполните это поле": "Please, fill this field",
		"Имя *": "Name *",
		"Телефон *": "Phone *",
		"Комментарий": "Comment",
		"Отправить": "Send",
		"Заявка на консультацию": "Request for consultation",
		"Наш менеджер свяжется с вами в ближайшее время": "Our manager will contact you soon",

		// terms.hbs
		"Мы используем файлы cookie, разработанные нашими специалистами и третьими лицами для анализа событий на нашем веб‑сайте, что позволяет нам улучшать взаимодействие с пользователями и обслуживание.":
			"",
		"Продолжая просмотр страниц нашего сайта, вы принимаете условия его использования.": "",
		"Более подробные сведения смотрите в нашей": "",
		"Политике конфиденциальности": "",
		"Мы используем файлы cookie": "",
		"Что это значит": "",
	}
});
